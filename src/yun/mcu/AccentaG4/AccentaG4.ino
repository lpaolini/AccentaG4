...
void startBridge() { Serial1.begin(LINK_SPEED); }

void stopBridge() { Serial1.end(); }

void checkBridge() {
    // Wait for the ready pin to go low, signaling that the AR3391 is fully
    // booted.
    if (digitalRead(READY_PIN)) {
        Serial.println("Bridge disabled");
        stopBridge();
        while (digitalRead(READY_PIN)) {
            // The pin is still high, so give the LED a quick flash to show
            // we're waiting.
...
void setupPanel() {
    ...
    // Give a delay for the AR3391 to reset, in case we were reset as part of a
    // reboot command. See
    // http://playground.arduino.cc/Hardware/Yun#rebootStability, case 3.
    delay(2500);
